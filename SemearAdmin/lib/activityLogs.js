...
const { ensureStructure } = require('./setup');

const SYSTEM_CODE = 'semear-admin';
const MENU_BY_PREFIX = [
    ['/api/module-users', 'Módulos'],
    ['/api/admin-users', 'Usuários do sistema'],
...
function buildActionLabel(method, menuLabel, body) {
    ...
    if (method === 'DELETE') return `${verb} um registro em ${menuLabel}`;
...
async function resolveActor(req) {
    if (!req.admin || !req.admin.id || !req.admin.tenant_id) return null;
...
async function writeLog(req) {
    try {
        ...
        console.error('Erro ao gravar log de atividade do SemearAdmin:', err);
    }
}
